...
const dateSchema = z.string().datetime('Invalid date format').or(z.date());
...
export const taxTreatmentEnum = z.enum(['taxable', 'tax_deferred', 'tax_free']);
...
export const bulkSyncSchema = z.object({
  ...
  scenarios: z
    .array(createScenarioSchema.extend({ id: uuidSchema }))
    .optional(),
  lastSyncTimestamp: dateSchema.optional(),
});
