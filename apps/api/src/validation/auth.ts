...
const passwordSchema = z
  ...
  .refine((password) => {
    ...
    const commonPatterns = ['123', 'abc', 'qwe', 'password', 'admin', '111', 'aaa'];
...
const nameSchema = z
  ...
  .refine((name) => {
    ...
    const xssPatterns = ['<script', '</script', 'javascript:', 'onload=', 'onerror=', 'onclick='];
...
const deviceInfoSchema = z
  ...
  .refine((deviceInfo) => {
    ...
    return !xssPatterns.some(pattern => deviceInfo.toLowerCase().includes(pattern));
...
export const oauthCallbackSchema = z.object({
  code: z.string().min(1, 'Authorization code is required').max(2000, 'Authorization code too long'),
  state: z.string().min(1, 'State parameter is required').max(100, 'State parameter too long').optional(),
  user: z.string().max(5000, 'User data too long').optional(), // For Apple OAuth
});

// Refresh token validation schema
export const refreshTokenSchema = z.object({
  refreshToken: z.string().min(1, 'Refresh token is required').max(2000, 'Refresh token too long'),
});

// Email verification validation schema
export const emailVerificationSchema = z.object({
  token: z.string().min(1, 'Verification token is required').max(100, 'Verification token too long'),
});
...
export const passwordResetSchema = z.object({
  token: z.string().min(1, 'Reset token is required').max(100, 'Reset token too long'),
  password: passwordSchema,
});
