...
export async function validateDatabaseSecurity(pool: Pool): Promise<void> {
  ...
  try {
    ...
    if (extensionsResult.rows.length > 0) {
      ...
      console.warn('⚠️  Potentially dangerous extensions found:', dangerousExtensions);
    }
...
export async function verifyDatabaseBackup(pool: Pool): Promise<void> {
  ...
  try {
    ...
    if (dbSize.size_bytes > 1024 * 1024 * 1024) { // 1GB
      console.warn('⚠️  Large database detected - ensure backup storage is adequate');
    }
...
export function startDatabaseSecurityMonitoring(pool: Pool): void {
  ...
  setInterval(() => {
    ...
    if (poolInfo.waitingCount > 0) {
      console.warn('⚠️  Database connection pool has waiting connections:', poolInfo);
    }
    
    // Log if pool is nearly exhausted
    if (poolInfo.idleCount === 0 && poolInfo.totalCount >= config.DB_POOL_MAX * 0.9) {
