...
export function loadSSLCertificates(): SSLConfig | null {
  try {
    ...
    if (!fs.existsSync(config.SSL_CERT_PATH)) {
      throw new Error(`SSL certificate file not found: ${config.SSL_CERT_PATH}`);
    }
...
export function httpsRedirectMiddleware(req: any, res: any, next: any) {
  if (config.FORCE_HTTPS && !req.secure && req.get('x-forwarded-proto') !== 'https') {
...
export function generateSelfSignedCert() {
  ...
  console.log('⚠️  Remember: Self-signed certificates will show security warnings in browsers');
