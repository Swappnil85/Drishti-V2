...
export class CertificateQueue {
      ...
      try {
        ...
        if (daysRemaining !== null && daysRemaining <= thresholdDays) {
          securityMonitor.recordEvent(
            ...
            daysRemaining <= 7 ? SecuritySeverity.CRITICAL : SecuritySeverity.HIGH,
            { host, daysRemaining, valid_to: info.valid_to, type: 'certificate_expiring' }
          );
        }
      } catch (error) {
        securityMonitor.recordEvent(
          ...
          { host, error: (error as Error).message, type: 'certificate_check_failed' }
        );
      }
      ...
      if (ctMonitor) {
        try {
          ...
          if (this.lastCtCount !== null && count > this.lastCtCount) {
            securityMonitor.recordSuspiciousActivity('ct_new_entries', { host, previous: this.lastCtCount, current: count });
          }
          this.lastCtCount = count;
        } catch (error) {
          securityMonitor.recordSuspiciousActivity('ct_check_failed', { host, error: (error as Error).message });
        }
      }
  ...
  async scheduleDaily() {
    ...
    const thresholdDays = parseInt(process.env.CERT_FRESHNESS_THRESHOLD_DAYS || '30', 10);
    const ctMonitor = (process.env.CT_MONITOR_ENABLED || 'true').toLowerCase() !== 'false';
    ...
    console.log(`📅 Daily certificate check scheduled for ${host} (threshold=${thresholdDays}d)`);
  }

  async shutdown() { await this.queue.close(); }
}
