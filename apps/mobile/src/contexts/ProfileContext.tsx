...
interface ProfileContextType {
  ...
  updateMultipleFields: (updates: UpdateProfileRequest[]) => Promise<UserProfile>;
...
export const ProfileProvider: React.FC<ProfileProviderProps> = ({ children }) => {
  ...
  const updateMultipleFields = async (updates: UpdateProfileRequest[]): Promise<UserProfile> => {
    try {
      setLoading(true);
      setError(null);
  ...
  const getRecommendations = async (): Promise<PersonalizedRecommendation[]> => {
    try {
      const recs = await ProfileService.getRecommendations();
      setRecommendations(recs);
      return recs;
  ...
  const dismissRecommendation = async (recommendationId: string, feedback?: string): Promise<void> => {
    try {
  ...
  const generateRecommendations = async (): Promise<PersonalizedRecommendation[]> => {
    try {
      const recs = await ProfileService.generateRecommendations();
      setRecommendations(recs);
      return recs;
  ...
  const exportProfileData = async (format: 'json' | 'csv' = 'json'): Promise<string> => {
    try {
      return await ProfileService.exportProfileData(format);
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'Failed to export profile data';
      setError(errorMessage);
      throw new Error(errorMessage);
    }
  };
  ...
  const hasRecommendations = recommendations.filter(rec => !rec.dismissed && !rec.accepted).length > 0;
