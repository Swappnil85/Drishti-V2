...
import { securityIntegrityService, IntegrityReport } from './SecurityIntegrityService';
...
export class DeviceIntegrityEnhanced {
  ...
  async getSecurityScore(forceRefresh = false): Promise<SecurityScoreData> {
    ...
    if (!forceRefresh && this.cachedScore && (now - this.lastCheck) < this.CACHE_DURATION) {
    ...
    const attestationEnabled = process.env.EXPO_PUBLIC_DEVICE_ATTESTATION_ENABLED === 'true';
      ...
      try {
        ...
        if (token) {
          ...
          if (result.success && result.verified) {
            attestationScore = Math.max(0, 100 - (result.riskScore || 50));
          }
        }
      } catch (error) {
    ...
    integrityReport.indicators.forEach(indicator => {
      ...
      factors.push({
        ...
        impact,
        description: indicator.details || (indicator.passed ? 'Security check passed' : 'Security concern detected')
      });
    });
  ...
  async getSecurityEducationContent(): Promise<Array<{title: string; content: string; priority: 'high' | 'medium' | 'low'}>> {
    const score = await this.getSecurityScore();
    const content = [];
    ...
    if (score.riskLevel === 'critical' || score.riskLevel === 'high') {
      content.push({
        ...
        content: 'Your device may be compromised. Consider using Drishti on a more secure device for financial data.',
    ...
    content.push({
      ...
      content: 'Regular security updates protect against known vulnerabilities. Enable automatic updates when possible.',
      priority: 'medium' as const
    });

    content.push({
      title: 'Use Strong Device Lock',
      content: 'Enable biometric authentication or a strong PIN/password to protect your device if lost or stolen.',
      priority: 'medium' as const
    });
